use iron::prelude::*;
use base::framework::{json_error_response, json_ok_response, not_found_response};
...
pub fn new(req: &mut Request) -> IronResult<Response> {
    ...
    validator.add_checker(Checker::new("article_id", Int, "文章ID") << Min(1))
    ...
    let comment_id =
        trans.prep_exec("INSERT INTO comment(article_id, user_id, content, create_time) VALUES (?, ?, ?, ?)",
                       (article_id, user.id, new_content, now))
            .unwrap()
            .last_insert_id();

    trans.prep_exec("UPDATE article set comments_count=comments_count+1, update_time=? where id=?",
                   (now, article_id))
        .unwrap();

    // send message to article's author
    if article_user_id != user.id {
        trans.prep_exec("INSERT INTO message(article_id, comment_id, from_user_id, to_user_id, mode, status, \
                        create_time) VALUES (?, ?, ?, ?, ?, ?, ?)",
                       (article_id,
                        comment_id,
                        user.id,
...
fn handle_mention(trans: &mut my::Transaction, username: &str) -> Option<u64> {
    ...
    trans.prep_exec("SELECT id from user where username=?", (username,))
        .unwrap()
        .next()
        .map(|row| my::from_row(row.unwrap()))
}
