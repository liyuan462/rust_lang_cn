use iron::prelude::*;
use base::framework::{ResponseData, temp_response, json_error_response, json_ok_response, json_redirect_response,
                      not_found_response};
...
pub fn register(req: &mut Request) -> IronResult<Response> {
    ...
    validator.add_checker(Checker::new("username", Str, "用户名") << Min(3) << Max(32) <<
    ...
    let mut stmt = pool.prepare("INSERT INTO user(username, email, password, salt, create_time) VALUES (?, ?, ?, ?, ?)")
        .unwrap();
...
pub fn github_register(req: &mut Request) -> IronResult<Response> {
    ...
    {
        let result =
            trans.prep_exec("INSERT INTO user(username, email, password, salt, create_time) VALUES (?, ?, ?, ?, ?)",
                            (username, email, "", "", now));
...
pub fn github_login(req: &mut Request) -> IronResult<Response> {
    ...
    let mut validator = Validator::new();
    validator.add_checker(Checker::new("username", Str, "用户名"))
...
pub fn login_load(req: &mut Request) -> IronResult<Response> {
    ...
    if let Some(refer) = req.headers.get::<Referer>() {
        ...
        if refer_url.starts_with(&app_path) && refer_url != app_path.clone() + "/user/login" &&
...
pub fn github_callback(req: &mut Request) -> IronResult<Response> {
    ...
    let mut stmt =
        pool.prepare("INSERT INTO github_user(id, username, email, avatar_url, create_time, update_time) VALUES (?, \
                      ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE username=VALUES(username), email=VALUES(email), \
                      avatar_url=VALUES(avatar_url), update_time=VALUES(update_time)")
            .unwrap();

    stmt.execute((github_user_id, github_user_name, github_user_email, github_user_avatar, now, now))
        .unwrap();

    // query whether is this github user binded to local user.
    let raw_user_id: Option<u64> = my::from_row(pool.prep_exec("SELECT user_id from github_user where id=?",
                   (&github_user_id,))
        .unwrap()
        .next()
        .unwrap()
    ...
    if let Some(user_id) = raw_user_id {
        ...
        if let Some(c) = req.get_cookie("redirect_url") {
            ...
            if redirect_url.starts_with(&app_path) && redirect_url != app_path.clone() + "/user/login" &&
...
pub fn show(req: &mut Request) -> IronResult<Response> {
    let user_id = try!(req.extensions
        .get::<Router>()
        .unwrap()
        .find("user_id")
        .unwrap()
    ...
    let articles: Vec<Article> =
        pool.prep_exec("SELECT id, category, title, content, comments_count, create_time from article where status=? \
                        and user_id=? order by create_time desc",
...
pub fn show_comments(req: &mut Request) -> IronResult<Response> {
    ...
    let comments: Vec<Comment> =
        pool.prep_exec("SELECT c.id, c.content, c.create_time, a.id as article_id, a.title as article_title from \
                        comment as c join article as a on c.article_id=a.id where c.user_id=? order by c.create_time \
                        desc",
                       (user_id,))
            .unwrap()
            .map(|x| x.unwrap())
            .map(|row| {
                let (id, content, create_time, article_id, article_title) = my::from_row(row);
...
pub fn show_messages(req: &mut Request) -> IronResult<Response> {
    ...
    let messages: Vec<Json> =
        pool.prep_exec("SELECT m.status, m.create_time, c.content, u.id as user_id, u.username, u.email, a.id as \
                        article_id, a.title as article_title from message as m join user as u on m.from_user_id=u.id \
                        join article as a on a.id=m.article_id join comment as c on c.id=m.comment_id where \
                        to_user_id=? order by create_time desc",
                       (user_id,))
            .unwrap()
            .map(|x| x.unwrap())
            .map(|row| {
                let (status, create_time, content, user_id, username, email, article_id, article_title) =
                    my::from_row::<(i8, NaiveDateTime, String, u64, String, String, u64, String)>(row);

                let mut object = Object::new();
                object.insert("is_new".to_owned(),
                              (if status == constant::MESSAGE::STATUS::INIT {
                                      true
                                  } else {
                                      false
                                  })
    ...
    pool.prep_exec("UPDATE message set status=? where to_user_id=? and status=?",
                   (constant::MESSAGE::STATUS::READ, user_id, constant::MESSAGE::STATUS::INIT))
        .unwrap();
...
fn get_general_info(data: &mut ResponseData,
                    pool: &my::Pool,
                    user_id: u64,
                    raw_login_user: Option<LoginUser>)
                    -> Result<(), ()> {
    let row = try!(pool.prep_exec("SELECT id, username, email, create_time from user where id=?",
                   (&user_id,))
        .unwrap()
        .next()
        .map(|row| row.unwrap())
    ...
    // get articles count
    let articles_count =
        my::from_row::<usize>(pool.prep_exec("SELECT count(id) from article where status=? and user_id=?",
                       (constant::ARTICLE::STATUS::NORMAL, user_id))
            .unwrap()
            .next()
            .unwrap()
    ...
    // get comments count
    let comments_count =
        my::from_row::<usize>(pool.prep_exec("SELECT count(id) from comment where user_id=?", (user_id,))
            .unwrap()
            .next()
            .unwrap()
            .unwrap());
    ...
    let which_member = my::from_row::<usize>(pool.prep_exec("SELECT count(id) as count from user where id < ?",
                   (user_id,))
        .unwrap()
        .next()
        .unwrap()
...
fn get_unread_messages_count(data: &mut ResponseData,
                             pool: &my::Pool,
                             user_id: u64,
                             raw_login_user: Option<LoginUser>) {
    ...
    let unread_messages_count =
        my::from_row::<usize>(pool.prep_exec("SELECT count(id) as count from message where to_user_id=? and status=?",
...
fn check_login(pool: &my::Pool, username: &str, password: &str) -> Option<u64> {
    let mut result = pool.prep_exec("SELECT id, email, password, salt from user where username=?",
                   (username,))
        .unwrap();
    ...
    let (user_id, _, pass, salt) = my::from_row::<(u64, String, String, String)>(row);
